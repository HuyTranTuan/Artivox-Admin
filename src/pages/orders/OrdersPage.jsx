...
import { Eye, Search, Filter, ChevronLeft, ChevronRight, X, ArrowUpDown } from "lucide-react";
...
const MOCK_ORDERS = [
  { id: "ORD-1847", code: "ORD-1847", customer: { name: "Nguyen Minh" }, totalAmount: 2400000, status: "DELIVERED", createdAt: "2026-05-05T10:30:00Z" },
  { id: "ORD-1846", code: "ORD-1846", customer: { name: "Tran Anh" }, totalAmount: 1800000, status: "PROCESSING", createdAt: "2026-05-04T14:20:00Z" },
  { id: "ORD-1845", code: "ORD-1845", customer: { name: "Le Hieu" }, totalAmount: 3200000, status: "SHIPPED", createdAt: "2026-05-03T09:15:00Z" },
  { id: "ORD-1844", code: "ORD-1844", customer: { name: "Pham Duc" }, totalAmount: 1200000, status: "PENDING", createdAt: "2026-05-02T16:45:00Z" },
  { id: "ORD-1843", code: "ORD-1843", customer: { name: "Hoang Tuan" }, totalAmount: 2800000, status: "DELIVERED", createdAt: "2026-05-01T11:00:00Z" },
  { id: "ORD-1842", code: "ORD-1842", customer: { name: "Vo Lan" }, totalAmount: 5600000, status: "PAID", createdAt: "2026-04-28T08:30:00Z" },
  { id: "ORD-1841", code: "ORD-1841", customer: { name: "Dang Khoa" }, totalAmount: 950000, status: "REFUND_PENDING", createdAt: "2026-04-25T13:10:00Z" },
  { id: "ORD-1840", code: "ORD-1840", customer: { name: "Bich Ngoc" }, totalAmount: 4100000, status: "DELIVERED", createdAt: "2026-04-22T15:25:00Z" },
];
...
const OrdersPage = () => {
  ...
  const statusOptions = [...new Set(orders.map((o) => o.status).filter(Boolean))];

  const filtered = useMemo(() => {
    let result = [...orders];
    const keyword = debouncedSearch.toLowerCase();
    if (keyword) {
      result = result.filter((o) => (o.code || o.id || "").toLowerCase().includes(keyword) || (o.customer?.name || o.customer || "").toLowerCase().includes(keyword));
    }
    if (selectedStatus) {
    ...
    <section className="space-y-6">
      <Card className="p-6">
        <div className="flex flex-col gap-4 mb-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h1 className="font-title text-2xl font-bold text-slate-950">{t("orders.title")}</h1>
          ...
          <div className="min-w-[900px]">
            <div className="overflow-hidden rounded-2xl border border-slate-200">
              <div className="grid grid-cols-[1.2fr_1.5fr_1fr_1fr_1fr_180px] gap-3 border-b border-slate-300 bg-slate-50 px-4 py-3 text-xs uppercase tracking-[0.2em] font-bold text-slate-900 sticky top-0 z-10">
                <div className="flex items-center gap-1 cursor-pointer" onClick={() => toggleSort("code")}>
                ...
                <div className="flex items-center gap-1 cursor-pointer" onClick={() => toggleSort("totalAmount")}>
              ...
              <div className="overflow-y-auto" style={{ maxHeight: "calc(100vh - 420px)" }}>
                {loading ? (
                  <div className="px-4 py-8 text-center text-sm text-slate-500">{t("orders.loading")}</div>
                ) : paginated.length === 0 ? (
                  <div className="px-4 py-8 text-center text-sm text-slate-500">{t("orders.noOrders")}</div>
                ) : (
                  paginated.map((item, idx) => (
                    ...
                    >
                      <div className="font-title text-sm font-semibold text-slate-900">{item.code || item.id}</div>
                      <div className="text-sm text-slate-700">{item.customer?.name || item.customer || "—"}</div>
                      <div className="font-semibold text-slate-900">{fmtPrice(item.totalAmount || item.amount)}</div>
                      <div>
        ...
        <div className="mt-6 flex items-center justify-between border-t border-slate-200 pt-6">
          <div className="text-sm text-slate-600">
            {t("orders.showing")} {filtered.length ? startIdx + 1 : 0}-{Math.min(startIdx + itemsPerPage, filtered.length)} {t("orders.of")} {filtered.length}
          </div>
