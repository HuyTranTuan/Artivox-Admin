...
import { Download, File, ImagePlus, Paperclip, SendHorizontal, X, Eye, ChevronLeft, PanelRightClose, PanelRightOpen } from "lucide-react";
...
const formatTime = (v) => new Date(v).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
const formatFileSize = (b) => b < 1024 ? `${b} B` : b < 1024 * 1024 ? `${(b / 1024).toFixed(1)} KB` : `${(b / (1024 * 1024)).toFixed(1)} MB`;
const downloadFile = (url, name) => { const a = document.createElement("a"); a.href = url; a.download = name; document.body.appendChild(a); a.click(); document.body.removeChild(a); };
const isImageType = (t) => t?.startsWith("image/");

const MessageContent = ({ message, onImageClick }) => {
  const [imgErr, setImgErr] = useState(false);
  const isImg = message.type === "image" || (message.type === "file" && isImageType(message.mimeType));
  const fileUrl = message.fileData || message.fileUrl || message.content;
  const name = message.fileName || message.content || "File";

  if (isImg && fileUrl && !imgErr) return (
    <div className="space-y-1.5">
      <div className="relative group rounded-lg overflow-hidden cursor-pointer border border-slate-200/50" onClick={() => onImageClick(fileUrl, name)}>
        <img src={fileUrl} alt={name} className="max-w-full max-h-48 object-contain bg-slate-100" onError={() => setImgErr(true)} />
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <button type="button" onClick={(e) => { e.stopPropagation(); downloadFile(fileUrl, name); }} className="inline-flex items-center gap-1 text-xs font-medium text-blue-500 hover:text-blue-700 shrink-0">
    ...
    <div className="space-y-1.5">
      <div className="flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          {message.fileSize && <div className="text-[10px] text-slate-400">{formatFileSize(message.fileSize)}</div>}
        </div>
        <button type="button" onClick={() => downloadFile(fileUrl || "#", name)} className="text-xs font-medium text-blue-500 hover:text-blue-700 shrink-0" disabled={!fileUrl}>
...
const ChatPage = () => {
  ...
  useEffect(() => {
    ...
    http.get(`/chat/rooms/${activeRoomId}/messages`)
      .then((data) => {
  ...
  useEffect(() => {
    ...
    const notifSocket = io(`${SOCKET_URL}/notifications`, { transports: ["websocket"] });
    ...
    chatSocket.on("chat:message", (msg) => {
      ...
      setConversations((prev) => ({ ...prev, [rid]: [...(prev[rid] || []), msg] }));
    });
  ...
  const activeMessages = (activeRoomId ? conversations[activeRoomId] : []) || [];
  const activeRoom = rooms.find((r) => String(r.id) === activeRoomId);
  ...
  const sendPayload = async (type, content, metadata = {}) => {
    ...
    // Optimistic
    const optimistic = { id: `tmp-${Date.now()}`, chatRoomId: activeRoomId, senderType: "ADMIN", type, content, timestamp: new Date().toISOString(), ...metadata };
    setConversations((prev) => ({ ...prev, [activeRoomId]: [...(prev[activeRoomId] || []), optimistic] }));
    ...
    try {
      ...
      setConversations((prev) => ({
        ...prev,
        [activeRoomId]: (prev[activeRoomId] || []).map((m) => m.id === optimistic.id ? savedMsg : m),
      }));
    } catch {
      setConversations((prev) => ({
        ...prev,
        [activeRoomId]: (prev[activeRoomId] || []).filter((m) => m.id !== optimistic.id),
      }));
    }
  };

  const handleSubmit = (e) => { e.preventDefault(); if (!message.trim()) return; sendPayload("text", message); setMessage(""); };
  ...
  return (
    <section className="space-y-6">
      ...
      <Card className="p-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <div className="mt-1 text-sm text-slate-500">{rooms.length} conversation{rooms.length !== 1 ? "s" : ""}</div>
          </div>
      ...
      <Card className="flex min-h-[70vh] flex-col overflow-hidden" style={{ minWidth: 500 }}>
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div className="flex items-center gap-3">
            <Button variant="ghost" className="h-9 w-9 p-0 rounded-lg" onClick={() => setCustomerListOpen(!customerListOpen)}>
            ...
            <div>
              <div className="font-title text-lg font-semibold text-slate-950">
                {activeRoom?.customer?.fullName || `Room ${activeRoomId}`}
              </div>
              <div className="text-xs text-slate-400">ID: {activeRoom?.customer?.id || activeRoom?.customerId}</div>
            </div>
          </div>
          <div className="text-xs text-slate-400">
            {activeMessages.length ? `Last: ${formatTime(activeMessages[activeMessages.length - 1].createdAt || activeMessages[activeMessages.length - 1].timestamp)}` : "No history"}
          </div>
        </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          {customerListOpen && (
            <div className="border-r border-slate-200 overflow-y-auto shrink-0 bg-slate-50/50" style={{ width: 250 }}>
              {loadingRooms ? (
                <div className="flex items-center justify-center h-20 text-sm text-slate-400">Loading…</div>
              ) : rooms.length === 0 ? (
                <div className="flex items-center justify-center h-20 text-sm text-slate-400">No conversations</div>
              ) : rooms.map((room) => {
                ...
                return (
                  ...
                  >
                    <div className="text-sm font-semibold text-slate-900 truncate">{room.customer?.fullName || `Customer ${room.customerId}`}</div>
                    <div className="text-xs text-slate-500 truncate mt-0.5">{last?.content || "No messages yet"}</div>
                  </button>
                );
              })}
            </div>
            ...
            <div className="flex-1 space-y-4 overflow-y-auto bg-slate-50/70 px-6 py-6">
              {activeMessages.length ? (
                activeMessages.map((item) => (
                  <div key={item.id} className={`flex ${item.senderType === "ADMIN" || item.sender === "admin" ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[70%] rounded-2xl px-4 py-3 ${
                      ...
                      <div className={`mt-2 text-[11px] ${item.senderType === "ADMIN" || item.sender === "admin" ? "text-white/70" : "text-slate-400"}`}>
            ...
            <form onSubmit={handleSubmit} className="border-t border-slate-200 bg-white px-6 py-4">
              <div className="flex items-end gap-3">
                <div className="flex flex-1 items-end gap-2">
                  <Input className="min-h-[48px]" placeholder="Type support reply…" value={message} onChange={(e) => setMessage(e.target.value)} disabled={!activeRoomId} />
                  <input ref={fileInputRef} type="file" className="hidden" onChange={(e) => handleFileSelect(e, "file")} />
                  <input ref={imageInputRef} type="file" accept="image/*" className="hidden" onChange={(e) => handleFileSelect(e, "image")} />
                  <Button type="button" variant="ghost" className="h-12 w-12 p-0" title="Attach file (max 15MB)" onClick={() => fileInputRef.current?.click()}><Paperclip style={{ width: 18, height: 18 }} /></Button>
                  <Button type="button" variant="ghost" className="h-12 w-12 p-0" title="Attach image (max 15MB)" onClick={() => imageInputRef.current?.click()}><ImagePlus style={{ width: 18, height: 18 }} /></Button>
                </div>
                <Button type="submit" className="h-12 gap-2 px-5" disabled={!message.trim() || !activeRoomId}>
                  <SendHorizontal style={{ width: 18, height: 18 }} /> Send
                </Button>
              </div>
            </form>
          </div>
