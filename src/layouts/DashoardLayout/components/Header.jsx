    ...
    <header className="sticky top-0 z-30 flex flex-col gap-4 border-b border-slate-200/70 bg-white/70 px-6 py-5 backdrop-blur lg:flex-row lg:items-center lg:justify-between">
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
        <div ref={search.containerRef} className="flex items-center gap-2">
          ...
          <Button variant="ghost" className="h-12 w-12 rounded-2xl p-0" onClick={search.submit}>
            <Search style={{ width: 26, height: 26 }} />
          </Button>
        </div>

        {/* Theme toggle */}
        <Button variant="ghost" className="h-12 w-12 rounded-2xl p-0" onClick={toggleTheme}>
          {theme === "light" ? <Moon style={{ width: 24, height: 24 }} /> : <Sun style={{ width: 24, height: 24 }} />}
        </Button>

        <div className="relative" ref={notificationRef}>
          <Button variant="ghost" className="relative h-12 w-12 rounded-2xl p-0" onClick={toggleNotifications}>
          ...
          {notificationOpen ? (
            <div className="absolute right-0 top-full mt-3 w-80 rounded-2xl border border-slate-200 bg-white px-[5px] py-[5px] shadow-xl" style={{ zIndex: 900 }}>
              <div className="flex items-center justify-between px-4 py-3">
                ...
                <button type="button" className="text-xs font-semibold text-amber-600 transition hover:text-amber-700" onClick={() => setShowUnreadOnly((prev) => !prev)}>
                ...
                {visibleNotifications.length ? (
                  visibleNotifications.map((item) => (
                    <div
                      key={item.id}
                      onClick={() => {
                        setNotifications((current) => current.map((notification) => (notification.id === item.id ? { ...notification, read: !notification.read } : notification)));
                      }}
        ...
        <div className="relative" ref={menuRef}>
          <button onClick={() => setUserMenuOpen(!userMenuOpen)} className="flex items-center gap-3 cursor-pointer focus:outline-none">
            ...
            <div className="hidden text-left lg:block">
              <div className="text-sm font-semibold text-slate-900 leading-tight">{user?.name || "User"}</div>
              <div className="text-xs text-slate-500 leading-tight">{user?.email || "user@artivox.vn"}</div>
            </div>
          </button>
